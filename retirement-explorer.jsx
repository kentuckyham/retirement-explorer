...
// ─────────────────────────────────────────────────────────────────────────────
const METHODS = [
  { id: "fixed",     name: "Fixed Real (Base Case)",     premium: 0,    std: 0,    spendEnd: "45/55",
    desc: "Withdraw a fixed inflation-adjusted amount each year. Simplest approach — no adjustments needed, but requires the lowest starting rate.",
    longDesc: "You choose a dollar amount in Year 1 (e.g. $200K), and withdraw that same amount every year, adjusted upward by inflation (2.5%/yr in this model). Your income is perfectly predictable, but the portfolio bears all the risk. If markets crash early, you keep withdrawing the same amount from a shrinking portfolio — which is why this method demands the lowest starting rate. Best for: retirees who prioritize income certainty above all else and can accept a lower starting income." },
  { id: "forgo",     name: "Forgo Inflation After Loss",  premium: 0.40, std: 5.5,  spendEnd: "48/52",
    desc: "Skip the inflation increase in any year following a portfolio loss. Minimal income volatility for a meaningful rate boost.",
    longDesc: "Works like Fixed Real, except in any year where the portfolio lost value, you skip the inflation adjustment and keep your withdrawal flat in nominal terms. This small concession — occasionally forgoing a 2-3% raise — meaningfully reduces the strain on the portfolio after bad years, allowing a higher starting rate. Your income never actually drops, it just sometimes doesn't grow. Over a 30-year retirement, you might skip the inflation adjustment 8-12 times. Best for: retirees who want near-fixed income but are willing to accept occasional flat years for a higher starting withdrawal." },
  { id: "rmd",       name: "RMD Method",                  premium: 0.82, std: 43.9, spendEnd: "93/7",
    desc: "Withdraw based on IRS life expectancy divisors. High income volatility — spending tracks the portfolio closely, leaving little bequest.",
    longDesc: "Each year, divide your portfolio by the number of remaining years in your planning horizon. Year 1 of a 40-year plan: withdraw 1/40th (2.5%). Year 20: withdraw 1/20th (5%). Year 39: withdraw 1/2 (50%). The method is mathematically incapable of depleting the portfolio — there's always something left to divide. However, your income swings wildly with market performance (44% standard deviation), and the method naturally accelerates withdrawals in later years. It spends ~93% of total wealth, leaving almost nothing as bequest. Best for: retirees who prioritize never running out over income stability and don't need to leave an inheritance." },
  { id: "guardrails",name: "Guardrails (Guyton-Klinger)",premium: 1.30, std: 28.9, spendEnd: "66/34",
    desc: "Cut spending 10% if rate exceeds 120% of initial; raise 10% if below 80%. Allows a high starting rate with structured adjustment rules.",
    longDesc: "Start with a chosen withdrawal rate, then adjust based on two guardrails: (1) If your current withdrawal rate rises above 120% of your initial rate (meaning the portfolio has dropped), cut spending by 10%. (2) If your current rate falls below 80% of initial (portfolio has grown), increase spending by 10%. This creates a self-correcting mechanism — you spend more in good times and less in bad times, but the adjustments are bounded and predictable. The structured rules allow a significantly higher starting rate than Fixed Real. Best for: retirees who can tolerate occasional 10% spending cuts in exchange for a much higher starting income, especially those with government benefits that create a spending floor." },
  { id: "decline",   name: "Actual Spending Decline",     premium: 1.10, std: 0,    spendEnd: "46/54",
    desc: "Assumes real spending naturally declines ~2%/yr in retirement (the 'go-go, slow-go, no-go' pattern). No volatility, but front-loads spending.",
    longDesc: "Based on research showing that retirees naturally spend less as they age — the 'go-go years' (active travel, dining), 'slow-go years' (less activity), and 'no-go years' (mostly home-based). This method sets a higher initial withdrawal that declines ~2% per year in real terms. By Year 20, you're spending about 67% of your initial amount in real terms. There's zero income volatility (the path is predetermined), but it front-loads your spending, which works well if you want to enjoy money while you're healthiest. Best for: retirees who expect to spend more in early retirement and naturally slow down later, and who want a predictable declining income path." },
  { id: "endowment", name: "Endowment (10yr Avg)",        premium: 1.80, std: 38.8, spendEnd: "58/42",
    desc: "Withdraw a fixed percentage of the 10-year rolling average portfolio value. Smooths volatility vs. constant percentage but still variable.",
    longDesc: "Withdraw a fixed percentage of your portfolio's 10-year rolling average value. By averaging over a decade, a single bad year doesn't immediately slash your income — the average absorbs the shock. However, prolonged downturns will eventually pull the average down, and strong recoveries take years to fully flow through. Income volatility (39%) is still high but smoother than Constant %. This is the approach used by many university endowments. Allows a high starting rate but income remains meaningfully variable. Best for: retirees comfortable with variable income who want some smoothing, and who have other income sources to cover essential expenses." },
  { id: "constant",  name: "Constant % of Balance",       premium: 1.80, std: 35.0, spendEnd: "58/42",
    desc: "Withdraw a fixed percentage of the current portfolio each year. Income rises and falls with the market — can never run out, but spending is volatile.",
    longDesc: "Withdraw the same percentage of whatever your portfolio is worth each year. If the portfolio drops 20%, your income drops 20%. If it doubles, your income doubles. Mathematically, this can never deplete the portfolio — you're always taking a fraction of a positive number. But the income volatility is severe (35% standard deviation). A $200K withdrawal could become $130K after a bad year. The method maximizes long-run spending efficiency but requires enormous flexibility in year-to-year lifestyle. Best for: retirees with very flexible spending needs, substantial guaranteed income from other sources, or those who view the portfolio as supplemental rather than essential." },
  { id: "vanguard",  name: "Vanguard Floor/Ceiling",      premium: 1.20, std: 36.4, spendEnd: "59/41",
    desc: "Apply a fixed percentage, but cap annual changes: max +5% increase, max −2.5% decrease. Balances responsiveness with stability.",
    longDesc: "A refinement of Constant % that limits how much your income can change year to year. Apply a fixed percentage to the current portfolio, but cap annual increases at +5% and decreases at −2.5%. This means after a 30% market crash, your income only drops 2.5% that year (not 30%). The downside: if markets stay down for several years, the 2.5% annual decreases compound. And in strong bull markets, the +5% cap means you don't fully participate in the upside. Despite the annual caps, long-run volatility is still high (36%) because small annual changes accumulate. Best for: retirees who like the idea of Constant % but want year-to-year stability, and who accept that the caps slow both recovery and participation." },
];
...
// ─────────────────────────────────────────────────────────────────────────────
const MC = {
  trials: 1000,          // trials per success-rate check
  eqVol:  0.17,          // equity annualized real std dev
  bondVol: 0.06,         // bond annualized real std dev
  targetSuccess: 0.90,   // 90% success threshold
  searchIter: 15,        // binary-search iterations (~0.001% precision)
};
...
// ── Strategy withdrawal functions ──
// Each takes a state object and returns the withdrawal for that year.
const WITHDRAW = {
  fixed: (s) => s.initialWD * Math.pow(1 + s.inflationRate, s.year - 1),

  forgo: (s) => {
    if (s.year === 1) return s.initialWD;
    return s.lastReturn < 0 ? s.prevWD : s.prevWD * (1 + s.inflationRate);
  },

  rmd: (s) => {
    const divisor = Math.max(1, s.horizon - s.year + 1);
    return s.portfolio / divisor;
  },

  guardrails: (s) => {
    if (s.year === 1) return s.initialWD;
    let wd = s.prevWD * (1 + s.inflationRate);  // inflation-adjust
    const rate = wd / s.portfolio;
    const initRate = s.initialWD / s.P0;
    if (rate > initRate * 1.2) wd *= 0.9;      // cut 10%
    else if (rate < initRate * 0.8) wd *= 1.1;  // raise 10%
    return wd;
  },

  decline: (s) => s.initialWD * Math.pow((1 + s.inflationRate) * 0.98, s.year - 1),

  endowment: (s) => {
    const pct = s.initialWD / s.P0;
  ...
  constant: (s) => {
    const pct = s.initialWD / s.P0;
    return pct * s.portfolio;
  },

  vanguard: (s) => {
    if (s.year === 1) return s.initialWD;
    const pct = s.initialWD / s.P0;
    const target = pct * s.portfolio;
    const ceil = s.prevWD * 1.05;
    const floor = s.prevWD * 0.975;
    return Math.min(ceil, Math.max(floor, target));
  },
};

// Run one trial: returns true if portfolio survives the full horizon
function mcTrial(methodId, startRate, horizon, equityPct, inflationRate, rng) {
  const P0 = 1_000_000;
  const initialWD = (startRate / 100) * P0;
  const eq = equityPct / 100;
  let portfolio = P0, prevWD = initialWD, lastReturn = 0;
  const history = [P0];
  const fn = WITHDRAW[methodId];

  for (let y = 1; y <= horizon; y++) {
    const zEq = boxMuller(rng), zBd = boxMuller(rng);
    const yearR = eq * (CMA.equity + MC.eqVol * zEq)
                + (1 - eq) * (CMA.bond + MC.bondVol * zBd);
    portfolio *= (1 + yearR);
    if (portfolio <= 0) return false;

    const wd = Math.max(0, fn({
      year: y, initialWD, P0, portfolio, prevWD, lastReturn, horizon, history, inflationRate,
    }));
    portfolio -= wd;
    // Depleted mid-plan = failure; depleted on the final year = success
    // (RMD, for example, withdraws everything in the last year by design)
    if (portfolio <= 0 && y < horizon) return false;
...
// Success rate for a given method + starting rate
function mcSuccessRate(methodId, startRate, horizon, equityPct, inflationRate, seed) {
  let ok = 0;
  for (let t = 0; t < MC.trials; t++) {
    if (mcTrial(methodId, startRate, horizon, equityPct, inflationRate, mulberry32(seed + t * 7919))) ok++;
  }
  return ok / MC.trials;
}

// Binary search for max safe rate at 90% success
// Returns { rate, capped } — capped=true means the strategy can sustain any rate (e.g. constant %)
function mcFindSafeRate(methodId, horizon, equityPct, inflationRate, seed) {
  let lo = 0.5, hi = 15.0;
  // Check if even the ceiling succeeds — if so, the method can't deplete
  if (mcSuccessRate(methodId, hi, horizon, equityPct, inflationRate, seed) >= MC.targetSuccess) {
    return { rate: hi, capped: true };
  }
  for (let i = 0; i < MC.searchIter; i++) {
    const mid = (lo + hi) / 2;
    if (mcSuccessRate(methodId, mid, horizon, equityPct, inflationRate, seed) >= MC.targetSuccess) lo = mid;
    else hi = mid;
  }
  return { rate: +lo.toFixed(2), capped: false };
}

// Run simulation for all 8 methods — returns { methodId: { safeRate, successRate, capped } }
function mcRunAll(horizon, equityPct, inflationRate) {
  const seed = 42;
  const results = {};
  for (const m of METHODS) {
    const { rate, capped } = mcFindSafeRate(m.id, horizon, equityPct, inflationRate, seed);
    results[m.id] = {
      safeRate: rate,
      successRate: capped ? 1.0 : mcSuccessRate(m.id, rate, horizon, equityPct, inflationRate, seed),
      capped,
    };
  }
  return results;
...
// ─────────────────────────────────────────────────────────────────────────────
// PORTFOLIO PROJECTION (strategy-aware)
// ─────────────────────────────────────────────────────────────────────────────
function buildProjection(inputs, methodId = "fixed") {
  const { portfolio, spending, retirementAge, lifeExpectancy, ssClaimAge, ssBenefit, cpp, oas, equityPct, inflation } = inputs;
  ...
  const withdrawFn = WITHDRAW[methodId];

  // Per-scenario state for variable withdrawal methods
  const sc = {};
  for (const key of Object.keys(RETURNS)) {
    sc[key] = { bal: portfolio, prevWD: spending, lastR: 0, hist: [portfolio] };
  }

  const data = [{ year: 0, age: retirementAge, pessimistic: portfolio, base: portfolio, optimistic: portfolio }];

  for (let y = 1; y <= years; y++) {
    const age = retirementAge + y;
    const benefitsYear = age >= ssClaimAge
      ? totalBenefits * Math.pow(1 + inflationRate, age - ssClaimAge)
      : 0;
    ...
    if (y === bridge && totalBenefits > 0) {
      ...
      for (const [key, r] of Object.entries(RETURNS)) {
        const s = sc[key];
        if (s.bal === null) { phantom[key] = null; continue; }
        const grown = s.bal * (1 + r);
        const wdState = { year: y, initialWD: spending, P0: portfolio, portfolio: grown,
          prevWD: s.prevWD, lastReturn: s.lastR, horizon: years, history: s.hist, inflationRate };
        const grossWD = Math.max(0, withdrawFn(wdState));
        const next = grown - grossWD;  // no benefit offset for phantom
    ...
    for (const [key, r] of Object.entries(RETURNS)) {
      const s = sc[key];
      if (s.bal === null) { point[key] = null; continue; }
      const grown = s.bal * (1 + r);
      const wdState = { year: y, initialWD: spending, P0: portfolio, portfolio: grown,
        prevWD: s.prevWD, lastReturn: s.lastR, horizon: years, history: s.hist, inflationRate };
      const grossWD = Math.max(0, withdrawFn(wdState));
      const netWD = Math.max(0, grossWD - benefitsYear);
      const next = grown - netWD;
      s.bal = next <= 0 ? null : next;
      s.prevWD = grossWD;
      s.lastR = r;
      s.hist.push(s.bal || 0);
      point[key] = s.bal === null ? 0 : s.bal;
    }
    data.push(point);
  }
  return data;
...
function renderMarkdown(text) {
  ...
  return blocks.map((block, bi) => {
    ...
    // Headings
    if (/^###\s/.test(trimmed)) {
      return React.createElement("h4", { key: bi, style: { fontSize: 13, fontWeight: 700, color: "#e2e8f0", margin: "12px 0 4px" } },
        renderInline(trimmed.replace(/^###\s+/, "")));
    }
    if (/^##\s/.test(trimmed)) {
      return React.createElement("h3", { key: bi, style: { fontSize: 14, fontWeight: 700, color: "#e2e8f0", margin: "14px 0 4px" } },
        renderInline(trimmed.replace(/^##\s+/, "")));
    }
    if (/^#\s/.test(trimmed)) {
      return React.createElement("h2", { key: bi, style: { fontSize: 15, fontWeight: 700, color: "#e2e8f0", margin: "14px 0 6px" } },
        renderInline(trimmed.replace(/^#\s+/, "")));
    }
    // Bullet list
    const lines = trimmed.split("\n");
    if (lines.every(l => /^[-*]\s/.test(l.trim()))) {
      return React.createElement("ul", { key: bi, style: { margin: "6px 0", paddingLeft: 18 } },
        lines.map((l, li) => React.createElement("li", { key: li, style: { marginBottom: 3 } },
          renderInline(l.trim().replace(/^[-*]\s+/, "")))));
    }
    // Numbered list
    if (lines.every(l => /^\d+[.)]\s/.test(l.trim()))) {
      return React.createElement("ol", { key: bi, style: { margin: "6px 0", paddingLeft: 18 } },
        lines.map((l, li) => React.createElement("li", { key: li, style: { marginBottom: 3 } },
          renderInline(l.trim().replace(/^\d+[.)]\s+/, "")))));
    }
    // Regular paragraph
    return React.createElement("p", { key: bi, style: { margin: "6px 0" } }, renderInline(trimmed));
  });
}
...
function renderInline(text) {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    if (match[2]) {
      parts.push(React.createElement("strong", { key: match.index, style: { fontWeight: 700, fontStyle: "italic", color: "#e2e8f0" } }, match[2]));
    } else if (match[3]) {
      parts.push(React.createElement("strong", { key: match.index, style: { fontWeight: 700, color: "#e2e8f0" } }, match[3]));
    } else if (match[4]) {
      parts.push(React.createElement("em", { key: match.index }, match[4]));
    }
...
function RetirementExplorer() {
  ...
  const [apiKey, setApiKey]         = useState(() => {
    try { return localStorage.getItem("re_apiKey") || ""; } catch { return ""; }
  });
  const [rememberKey, setRememberKey] = useState(() => {
    try { return localStorage.getItem("re_rememberKey") === "true"; } catch { return false; }
  });
  const [analysis, setAnalysis]     = useState("");
  const [analyzing, setAnalyzing]   = useState(false);
  const [apiError, setApiError]     = useState("");
  const [mcResults, setMcResults]   = useState(null);      // { methodId: { safeRate, successRate } }
  const [mcRunning, setMcRunning]   = useState(false);
  const [mcInputs, setMcInputs]    = useState(null);       // snapshot of inputs when MC was run
  const [chartMethod, setChartMethod] = useState("fixed"); // strategy for projection chart
  const [mcView, setMcView]         = useState("mc");      // "mc" | "est" — which data to show in methods table
  const [hoveredInfo, setHoveredInfo] = useState(null);     // method id for tooltip
  ...
  function handleApiKeyChange(val) {
    ...
    setApiError("");
    try {
      if (rememberKey) localStorage.setItem("re_apiKey", val);
    } catch {}
  }
  ...
  function handleRememberToggle(checked) {
    ...
    try {
      if (checked) {
        localStorage.setItem("re_rememberKey", "true");
        localStorage.setItem("re_apiKey", apiKey);
      } else {
        localStorage.removeItem("re_rememberKey");
        localStorage.removeItem("re_apiKey");
      }
    } catch {}
  }
  ...
  const chartData      = useMemo(() => buildProjection(inp, chartMethod), [inp, chartMethod]);
  const selectedMethod = METHODS.find(m => m.id === chartMethod);
  ...
  function buildAnalysisPrompt() {
    ...
    const baseContext = `You are a retirement planning analyst grounding your analysis in Morningstar's State of Retirement Income 2025 research.
...
- Net portfolio draw after benefits: $${netDraw.toLocaleString()}/year (${adjustedRate.toFixed(1)}%)`;
    ...
    if (activeTab === "methods") {
      // Build MC results summary if available
      let mcSummary = "";
      const useMC = mcResults && !mcStale && mcView === "mc";
      if (useMC) {
        mcSummary = "\n\nMonte Carlo simulation results (1,000 trials, 90% success threshold):";
        for (const m of METHODS) {
          const r = mcResults[m.id];
          const isFormulaDriven = m.id === "rmd" || m.id === "constant";
          mcSummary += `\n- ${m.name}: ${isFormulaDriven ? "N/A (cannot deplete)" : r.safeRate.toFixed(1) + "%"} safe rate, ${(r.successRate * 100).toFixed(0)}% success, income volatility ${m.std}%, spend/bequest ${m.spendEnd}`;
        }
      } else {
        mcSummary = "\n\nMorningstar-estimated safe rates (premium over base case):";
        for (const m of METHODS) {
          const mRate = getMethodRate(m, inp.equityPct, horizon);
          const isFormulaDriven = m.id === "rmd" || m.id === "constant";
          mcSummary += `\n- ${m.name}: ${isFormulaDriven ? "N/A (cannot deplete)" : mRate.toFixed(1) + "%"} safe rate, income volatility ${m.std}%, spend/bequest ${m.spendEnd}`;
        }
      }

      return baseContext + mcSummary + `

The user is comparing all 8 withdrawal strategies. Write a 2\u20133 paragraph plain-language analysis. Cover:
1. Which 2\u20133 strategies are the best fit for THIS specific scenario and why \u2014 consider the bridge period, government income offset, and spending needs
2. The key trade-off the user faces (income stability vs. higher starting income) given their numbers
3. One specific insight about how government benefits at age ${inp.ssClaimAge} interact with the variable-income strategies (Guardrails, Vanguard, etc.)

Be direct and specific. Reference actual rates and dollar amounts. No boilerplate. Do not give investment advice or say "consult a financial advisor."`;
    }

    // Scenario Analysis tab prompt
    return baseContext + `
...
Be direct and specific. No boilerplate. Reference actual numbers. Do not give investment advice or say "consult a financial advisor."`;
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (!res.ok) {
          ...
          if ((res.status === 529 || res.status === 429 || /overloaded/i.test(msg)) && attempt < maxRetries) {
        ...
        if (attempt < maxRetries && /overloaded|rate/i.test(err.message)) {
          await new Promise(r => setTimeout(r, 2000 * (attempt + 1)));
          continue;
        }
  ...
  function handleRunMC() {
    setMcRunning(true);
    // Use setTimeout so the "Running…" state renders before the sync computation
    setTimeout(() => {
      try {
        const inflationRate = (inp.inflation || 2.5) / 100;
        const results = mcRunAll(horizon, inp.equityPct, inflationRate);
        setMcResults(results);
        setMcInputs({ equityPct: inp.equityPct, horizon, inflation: inp.inflation });
        setMcView("mc");
      } catch (err) {
  ...
  // Check if MC results are stale (inputs changed since last run)
  const mcStale = mcResults && mcInputs &&
    (mcInputs.equityPct !== inp.equityPct || mcInputs.horizon !== horizon || mcInputs.inflation !== inp.inflation);
    ...
    <div style={{ fontFamily: "'Inter', -apple-system, BlinkMacSystemFont, sans-serif",
      ...
      <div style={{ display: "flex", background: "#141829", border: "1px solid #1e2540",
        ...
        <div style={{ width: "37%", borderRight: "1px solid #1e2540", padding: 20,
          ...
          <Slider label="Portfolio Size"    value={inp.portfolio}      onChange={v => set("portfolio", v)}      min={500_000} max={10_000_000} step={25_000}  fmt={fmtDollars} />
          ...
          <Slider label="Assumed Inflation Rate" value={inp.inflation} onChange={v => set("inflation", v)}
            min={0} max={6} step={0.1} fmt={v => `${v.toFixed(1)}%`} />
          <div style={{ fontSize: 10, color: "#64748b", marginTop: -8, marginBottom: 12, lineHeight: 1.4 }}>
            Affects inflation-linked strategies (Fixed Real, Forgo, Guardrails, Decline) and
            government benefit growth. Higher inflation reduces purchasing power of fixed withdrawals.
          </div>
          ...
          {/* AI Analysis — tab-aware */}
          <div style={{ background: "#141829", border: "1px solid #1e2540", borderRadius: 8,
            padding: "14px 16px", marginTop: 16 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
              <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: "0.06em",
                textTransform: "uppercase", color: "#7a8499" }}>AI Analysis</div>
            ...
            <div style={{ display: "flex", gap: 6, marginBottom: 6 }}>
              <input type="password" value={apiKey} placeholder="sk-ant-..."
                ...
                style={{ flex: 1, background: "#0f1220", border: "1px solid #1e2540",
                  borderRadius: 6, padding: "7px 10px", fontSize: 12, color: "#e2e8f0" }} />
              <button onClick={handleAnalyze} disabled={analyzing} style={{
                background: analyzing ? "#475569" : "linear-gradient(135deg, #3b6cf6, #5a4cf6)",
                color: "white", border: "none", borderRadius: 6, padding: "7px 14px",
                fontSize: 12, fontWeight: 600,
                cursor: analyzing ? "not-allowed" : "pointer", whiteSpace: "nowrap",
                boxShadow: analyzing ? "none" : "0 2px 8px rgba(59, 108, 246, 0.25)",
              }}>
                {analyzing ? "Analyzing\u2026" : activeTab === "analysis" ? "Analyze Scenario" : "Compare Methods"}
              </button>
            </div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between",
              ...
              <label style={{ display: "flex", alignItems: "center", gap: 6, cursor: "pointer" }}>
                <input type="checkbox" checked={rememberKey}
                  ...
                  style={{ accentColor: "#3b6cf6", cursor: "pointer" }} />
                <span style={{ fontSize: 11, color: "#8892a8" }}>Remember on this device</span>
              </label>
              <a href="https://console.anthropic.com" target="_blank" rel="noreferrer"
                style={{ fontSize: 10, color: "#3b6cf6" }}>
                Get a key &rarr;
              </a>
            </div>
            <p style={{ fontSize: 10, color: "#64748b", marginBottom: 8, lineHeight: 1.5 }}>
              {rememberKey
                ? "Key saved in your browser\u2019s local storage. Never sent anywhere except Anthropic\u2019s API."
                : "Key is not stored. You\u2019ll need to paste it each session."}
            </p>
            {apiError && (
              <div style={{ background: "rgba(239,68,68,0.08)", border: "1px solid rgba(248,113,113,0.3)",
                borderRadius: 6, padding: "6px 10px", fontSize: 11, color: "#f87171", marginBottom: 8 }}>
                {apiError}
              </div>
            )}
            {(analysis || analyzing) && (
              <div style={{ background: "#0f1220", border: "1px solid #1e2540", borderRadius: 6,
                padding: 10, fontSize: 12, color: "#8892a8", lineHeight: 1.7, maxHeight: 300, overflowY: "auto" }}>
                {analysis
                  ? <div>{renderMarkdown(analysis)}</div>
                  : <span style={{ color: "#3d4563", fontStyle: "italic" }}>Analyzing{"\u2026"}</span>
                }
              </div>
            )}
          </div>
        ...
        <div style={{ flex: 1, padding: 20, overflowY: "auto" }}>
          ...
          <div style={{ marginBottom: 10 }}>
            <MetricCard
              label="Bridge Period &middot; highest-risk window &middot; no government income"
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10, marginBottom: 16 }}>
            <MetricCard
              label="During Bridge &middot; before benefits"
          ...
          <div style={{ background: "#141829", border: "1px solid #1e2540",
            ...
            <div style={{ background: "#0c0f1a", borderRadius: 4, padding: "6px 10px", marginTop: 8,
              ...
              disproportionately harmful. A strategy that looks safe here may still fail under volatile returns.
              See <strong>Compare Methods</strong> for starting-rate analysis that accounts for this risk.
            </div>
          </div>
          ...
          {activeTab === "methods" && <>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <button onClick={handleRunMC} disabled={mcRunning} style={{
                    ...
                    {mcRunning ? "Running simulation\u2026" : mcResults && !mcStale ? "Re-run Monte Carlo" : "Run Monte Carlo (1,000 trials)"}
                  </button>
                </div>
              </div>
              <p style={{ fontSize: 12, color: "#64748b", lineHeight: 1.6, marginBottom: 8 }}>
                ...
                for your current inputs: {inp.equityPct}% equity, {horizon}-year horizon.
              ...
              {mcResults && !mcStale && mcView === "mc" && (
                <div style={{ background: "rgba(34, 197, 94, 0.06)", border: "1px solid #6ee7b7", borderRadius: 6,
                  ...
                  bonds {(CMA.bond*100).toFixed(1)}% &plusmn; {(MC.bondVol*100).toFixed(0)}% vol.
                </div>
              )}
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 16 }}>
              ...
              />
              <MetricCard
                label={`Base Starting Rate \u00B7 ${horizon}yr`}
                value={fmtPct(mcResults && !mcStale && mcView === "mc" ? mcResults.fixed.safeRate : safeRate)}
                sub={mcResults && !mcStale && mcView === "mc" ? "Monte Carlo \u00B7 Fixed Real" : "Morningstar table \u00B7 Fixed Real"}
                valueColor="#60a5fa"
              />
              <MetricCard label="Methods in Range">
                <div style={{ fontSize: 22, fontWeight: 800, color: "#4ade80", lineHeight: 1, marginBottom: 4 }}>
                  {METHODS.filter(m => {
                    ...
                    const r = mcResults && !mcStale && mcView === "mc" ? mcResults[m.id].safeRate : getMethodRate(m, inp.equityPct, horizon);
            ...
            <div style={{ background: "#141829", border: "1px solid #1e2540", borderRadius: 8, overflow: "hidden" }}>
              ...
              {METHODS.map((m, i) => {
                const useMC = mcResults && !mcStale && mcView === "mc";
                const isCapped = useMC && mcResults[m.id].capped;
                // RMD and Constant % are formula-driven — no user-chosen starting rate
                const isFormulaDriven = m.id === "rmd" || m.id === "constant";
                const mRate = useMC ? mcResults[m.id].safeRate : getMethodRate(m, inp.equityPct, horizon);
                // For formula-driven strategies, show actual Year-1 withdrawal regardless of MC state
                let yr1, yr1Note;
                if (isFormulaDriven && m.id === "rmd") {
                  yr1 = inp.portfolio / horizon;  // RMD: portfolio ÷ remaining years
                  yr1Note = `portfolio \u00F7 ${horizon}yr`;
                } else if (isFormulaDriven && m.id === "constant") {
                  const reasonablePct = getExpectedReturn(inp.equityPct) * 100;
                  yr1 = (reasonablePct / 100) * inp.portfolio;
                  yr1Note = `${reasonablePct.toFixed(1)}% of balance`;
                } else {
                  yr1 = (mRate / 100) * inp.portfolio;
                  yr1Note = null;
                }
                const isBase = m.premium === 0;
                // For formula-driven methods, check if actual Year-1 withdrawal covers spending
                const canSupport = isFormulaDriven ? yr1 >= inp.spending : mRate >= withdrawalRate;
                return (
                  <div key={m.id} style={{
                    ...
                    <div style={{ textAlign: "right" }}>
                      ...
                      {isFormulaDriven
                        ? <div style={{ fontSize: 10,
                            color: canSupport ? "#6ee7b7" : yr1 >= inp.spending * 0.8 ? "#fbbf24" : "#f87171" }}>
                            can{"\u2019"}t deplete
                          </div>
                        : useMC
            ...
            <div style={{ marginTop: 12, padding: "12px 14px", background: "rgba(59, 108, 246, 0.06)",
              ...
              Higher-rate strategies come with trade-offs &mdash; mainly income volatility
            ...
            {mcResults && !mcStale && mcView === "mc" && (
              <div style={{ marginTop: 12, padding: "12px 14px", background: "#0f1220",
                ...
                Portfolio returns are weighted by allocation. A trial "fails" if the portfolio hits zero before the
                planning horizon. The safe rate is the highest starting rate achieving {"\u2265"}90% survival.
                Results are deterministic (seeded PRNG) so re-running with the same inputs produces identical output.
              </div>
            )}
