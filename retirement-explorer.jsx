  ...
  for (let y = 1; y <= years; y++) {
    ...
    if (y === bridge && totalBenefits > 0) {
      ...
      for (const [key, r] of Object.entries(RETURNS)) {
        ...
        const noBenefitWD = inflatedSpend;  // full spend, no offset
        ...
        <div style={{ flex: 1, padding: 20, overflowY: "auto" }}>
          ...
          <div style={{ background: "white", border: "1px solid #e2e8f0",
            ...
            <p style={{ fontSize: 10, color: "#94a3b8", fontStyle: "italic", marginTop: 6 }}>
              ...
              End-of-year withdrawal convention. Actual sequence of returns matters more than averages &mdash;
          ...
          {activeTab === "methods" && <>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <p style={{ fontSize: 12, color: "#64748b", lineHeight: 1.6, marginBottom: 8 }}>
                ...
                {mcResults && !mcStale
                  ...
                  : " Rates estimated via premium extrapolation (click Run Monte Carlo for simulation-based rates)."}
              </p>
              {mcStale && (
                <div style={{ background: "#fef3c7", border: "1px solid #fcd34d", borderRadius: 6,
              ...
              {!mcResults && (
                <div style={{ background: "#fef3c7", border: "1px solid #fcd34d", borderRadius: 6,
                  ...
                  30-year / 40% equity data. Run Monte Carlo for simulation-based rates specific to your inputs.
                </div>
              )}
              {mcResults && !mcStale && (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 16 }}>
              ...
              <MetricCard
                ...
                value={fmtPct(mcResults && !mcStale ? mcResults.fixed.safeRate : safeRate)}
                sub={mcResults && !mcStale ? "Monte Carlo \u00B7 Fixed Real" : "Morningstar table \u00B7 Fixed Real"}
                valueColor="#60a5fa"
              />
              <MetricCard label="Methods in Range">
                <div style={{ fontSize: 22, fontWeight: 800, color: "#22c55e", lineHeight: 1, marginBottom: 4 }}>
                  {METHODS.filter(m => {
                    const r = mcResults && !mcStale ? mcResults[m.id].safeRate : getMethodRate(m, inp.equityPct, horizon);
            ...
            <div style={{ background: "white", border: "1px solid #e2e8f0", borderRadius: 8, overflow: "hidden" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr 1fr",
                ...
                <div>Strategy</div>
                <div style={{ textAlign: "right" }}>{mcResults && !mcStale ? "MC Starting Rate" : "Est. Starting Rate"}<br/><span style={{ fontWeight: 400, fontSize: 9, letterSpacing: 0, textTransform: "none" }}>(year-1 as % of portfolio)</span></div>
              ...
              {METHODS.map((m, i) => {
                ...
                // For "can't deplete" strategies, show actual formula-based Year-1 withdrawal
                let yr1, yr1Note;
                if (isCapped && m.id === "rmd") {
                  yr1 = inp.portfolio / horizon;  // RMD: portfolio ÷ remaining years
                  yr1Note = `portfolio \u00F7 ${horizon}yr`;
                } else if (isCapped && m.id === "constant") {
                  // Constant % is always sustainable; show a reasonable starting % (e.g. expected return)
                ...
                const canSupport = isCapped || mRate >= withdrawalRate;
                return (
                  <div key={m.id} style={{
                    ...
                    <div style={{ textAlign: "right" }}>
                      <div style={{ fontSize: 15, fontWeight: 800,
                        color: isCapped ? "#22c55e" : canSupport ? "#22c55e" : withdrawalRate <= mRate + 0.5 ? "#f59e0b" : "#ef4444" }}>
                        {isCapped ? "N/A" : `${mRate.toFixed(1)}%`}
                      </div>
                      {useMC
                        ? <div style={{ fontSize: 10, color: "#6ee7b7" }}>
                            {isCapped ? "can\u2019t deplete" : `MC ${(mcResults[m.id].successRate * 100).toFixed(0)}% success`}
                          </div>
